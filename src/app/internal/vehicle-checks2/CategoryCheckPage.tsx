...
const vehicleCheckStatusStorageKey = "hgv-backup2-vehicle-check-status";
...
export default function CategoryCheckPage({
  ...
  checks,
  storageKeyPrefix,
}: CategoryCheckPageProps) {
  const statusStorageKey = `${storageKeyPrefix}-status`;
  const descriptionsStorageKey = `${storageKeyPrefix}-descriptions`;
  const photoNamesStorageKey = `${storageKeyPrefix}-photo-names`;
  ...
  useEffect(() => {
    ...
    const savedPhotoNames = window.localStorage.getItem(photoNamesStorageKey);
  ...
  }, [descriptionsStorageKey, photoNamesStorageKey, statusStorageKey]);
  ...
  function getOverallCategoryStatus(): CheckStatus {
    const currentStatuses = checks.map(
  ...
  function saveAndReturnToVehicleChecks() {
    const overallCategoryStatus = getOverallCategoryStatus();
    ...
    const nextVehicleStatuses: Record<number, CheckStatus> = {
      ...
      [categoryNumber]: overallCategoryStatus,
    };
    ...
    window.location.href = "/internal/vehicle-checks2";
  }
  ...
  function resetMockup() {
    ...
    window.localStorage.removeItem(descriptionsStorageKey);
    window.localStorage.removeItem(photoNamesStorageKey);
      ...
      <section className="bg-[#b00020] px-4 py-7 text-white sm:px-6 lg:px-10">
        <div className="mx-auto max-w-[900px]">
          <p className="mb-3 text-xs font-black uppercase tracking-[0.22em] text-[#ffd9df]">
            Vehicle check category {categoryNumber}
          </p>
