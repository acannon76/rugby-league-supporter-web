...
export default function VehicleChecksAltPage() {
  ...
  function completeVehicleChecks() {
    ...
    const storedStartTimestamp = Number(
      window.localStorage.getItem("hgv-backup2-check-timer-started-at")
    );
    ...
    altCheckCategories.forEach((category) => {
      window.localStorage.removeItem(`hgv-backup2-alt-category-state-${category.slug}`);
    });
    setStatuses({});
    setCurrentMileage("");

    router.push("/internal/logbook2");
  }
...
function CheckRow({
  ...
  return (
    <div className="grid grid-cols-[1fr_74px] gap-3">
      <Link
        href={`/internal/vehicle-checks-alt2/${check.slug}`}
